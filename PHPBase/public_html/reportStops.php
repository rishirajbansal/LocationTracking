...
$locationHistoryWorkerStopsBased = NULL;
...
if (isset($_POST['search'])){
    ...
    if ($action['result'] != 'error'){
        ...
        $flag = $locationHistoryDAO->fetchLocationHistoryBasedOnStops($inputs, $user);
        ...
        if (!$flag){
            ...
            $locationHistoryWorkerStopsBased = $locationHistoryDAO->getLocationHistoryWorkerStopsBased();
            $_SESSION['reportData'] = $locationHistoryWorkerStopsBased;
    ...
    header('location: downloadReport.php?report=stopsSummary&type='.$_POST['exportType']);
}
...
<html lang="en">
    <head>
        <meta charset="utf-8">
        <title>Location History &middot; Stops Based Summary</title>
    ...
    <body>
        ...
        <header class="jumbotron subhead" id="overview">
            <div class="container" style="padding-left: 35px;margin-left: inherit"> 
              <h1 style="font-size:27px">Worker Location History - Stops based Summary</h1>
              <p class="lead" style="font-size:17px">Report to get the records of stop positions based on the selected time interval</p>
           </div>
        </header>
                        ...
                        <div class="hero-unit" style="background-color: #FFFFFF; min-height: 400px; padding: 20px">
                            ...
                            if ($action['result'] == 'success') { ?>
                                ...
                                <div class="span12" style="margin-left: 0px">
                                    ...
                                    <table class="table table-striped" style="font-size: 14px;margin-bottom: 0px;">
                                        <tbody>
                                            <tr>
                                                <td width="15%" style="background-color: #AD3A4F;color: #FFFFFF;font-weight: bold;font-size: 18px;text-align: left">Last recorded time : </td>
                                                <td style="background-color: #E7D2D6;font-weight: bold;font-size: 18px;text-align: left;border-right: 20px solid #FFFFFF"><?php echo $locationHistoryWorkerStopsBased['lastRecordedTime'];?></td>
                                                <td width="25%" style="background-color: #FFFFFF;border: none;"></td>
                                                <td width="15%" style="background-color: #AD3A4F;color: #FFFFFF;font-weight: bold;font-size: 18px;text-align: left;border-left: 20px solid #FFFFFF">Total Stop Time : </td>
                                                <td style="background-color: #E7D2D6;font-weight: bold;font-size: 18px;text-align: left"><?php echo $locationHistoryWorkerStopsBased['totalDuration'];?></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                    ...
                                    <table class="table table-bordered table-striped" style="font-size: 14px;">
                                        ...
                                        <tbody>
                                            <?php
                                                $ctr = 1;
                                                $rowToggle=0;
                                                $locationHistoryList = $locationHistoryWorkerStopsBased['locationHistoryList'];
                                                foreach ($locationHistoryList as $locationHistory) { ?>
                                                <tr <?php if ($rowToggle) { $rowToggle=0?>class="warning" <?php } else { ?> class="" <?php $rowToggle=1; } ?>>
                                                       ...
                                                       <td><?php echo $locationHistory->getStopDuration();?></td>
